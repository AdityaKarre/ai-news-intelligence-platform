...
import { RotateCw, ArrowUp, Layers, Zap, ExternalLink, ChevronDown, X } from 'lucide-react';
...
export default function Home() {
  ...
  // Fetch news data directly from our Next.js backend endpoint API
  const fetchNewsFeed = async (currentRegion, currentCategory, isRefreshAction = false) => {
    if (isRefreshAction) setIsRefreshing(true);
    else setIsLoading(true);

    try {
      const response = await fetch(`/api/news?region=${currentRegion}&category=${currentCategory}`);
  ...
  // 🛡️ ADVANCED HIERARCHICAL HARDWARE BACK-BUTTON INTERCEPT STACK
  useEffect(() => {
    ...
    <div className="relative min-h-screen bg-[#070514] text-slate-100 font-sans selection:bg-purple-500/30 selection:text-purple-200 overflow-x-hidden">
      ...
      <nav className="sticky top-0 z-40 w-full border-b border-white/[0.03] bg-[#070514]/60 backdrop-blur-lg">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 h-16 flex items-center justify-between">
          <div 
            onClick={() => {
              setHasExplored(false); // Global Escape Hatch: Returns instantly to the Home Page landing screen
              setRegion('India');
              setCategory('all');
              setSelectedArticle(null);
              window.scrollTo({ top: 0, behavior: 'smooth' });
            }}
            className="text-lg font-black tracking-widest cursor-pointer bg-gradient-to-r from-white via-slate-200 to-purple-400 bg-clip-text text-transparent hover:opacity-80 transition"
          >
                  ...
                  >
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2.5 mb-2">
                        <span className="text-[9px] font-black uppercase tracking-widest px-2 py-0.5 rounded bg-purple-500/10 text-purple-300 border border-purple-500/10">
                          {article.category}
                        </span>
      ...
      {/* 🔮 4. INTELLIGENCE VIEW MODAL (Responsive Adaptation Layout) */}
      <AnimatePresence>
        {selectedArticle && (
          <div 
          ...
          >
            ...
            >
                  ...
                  {!showDeepContext ? (
                    ...
                    >
                      ...
                      <div className="space-y-3.5 text-xs sm:text-sm leading-relaxed text-slate-300">
                        ...
                        <div className="pt-3 border-t border-white/[0.02] text-xs italic text-teal-400/90 bg-teal-500/[0.01] p-3 rounded-lg border border-teal-500/5 break-words">
                          <span className="font-bold uppercase tracking-wider not-italic block text-[10px] text-teal-400 mb-1">Strategic Intelligence:</span>
                          {selectedArticle.ai_insight}
                        </div>
                      </div>
                    </motion.div>
                  )}
              ...
              {/* TOGGLE CONTROLLER BUTTON (Placed below active content pane) */}
              <button
      ...
      {/* 🚀 5. FLOATING SMOOTH BACK TO TOP BUTTON */}
      <AnimatePresence>
